...
class BiCycleGANModel(BaseModel):
    def __init__(self, opt):
        ...
        self.loss_names = ['G_GAN', 'D', 'G_GAN2', 'D2', 'G_L1', 'G_CE', 'z_L1', 'kl', 'kl_rev']
    ...
    def set_input(self, input):
        ...
        self.real_B_rev = input['B_rev' if AtoB else 'A_rev'].to(self.device)
    ...
    def test(self, z0=None, encode=False, turbulent=False, turbt_ratio=1., zeros=False):
        with torch.no_grad():
            ...
            if zeros:
                z0 = 0 * z0
            if turbulent:
                z0 = z0 + self.get_z_random(self.real_A.size(0), self.opt.nz) * turbt_ratio
    ...
    def forward(self):
        ...
        self.real_B_rev_encoded = self.real_B_rev[0:half_size]
        ...
        self.z_rev_encoded, self.mu_rev, self.logvar_rev = self.encode(self.real_B_rev_encoded)
        # get random z
    ...
    def backward_D(self, netD, real, fake):
        ...
        if self.opt.gumble_softmax:
            fake = F.gumbel_softmax(fake, tau=1, hard=True, dim=1)
    ...
    def backward_EG(self):
        ...
        if self.opt.lambda_kl > 0.0:
            ...
            self.loss_kl = 0
        if self.opt.lambda_kl_rev > 0.0:
            # kl with gaussian distribution
            self.loss_kl_rev = torch.sum(1 + self.logvar_rev - self.mu_rev.pow(2) - self.logvar_rev.exp()) * (
                    -0.5 * self.opt.lambda_kl_rev)
            # kl with reversed trend
            self.loss_kl_rev += torch.sum(1 + self.logvar - self.logvar_rev - (1 / self.logvar_rev.exp()) * (
                    self.logvar.exp() + (self.mu - self.mu_rev).pow(2))) * (-0.5 * self.opt.lambda_kl_rev)
        else:
            self.loss_kl_rev = 0
        ...
        if self.opt.lambda_L1 > 0.0:
            ...
            self.loss_G_L1 = 0.0
        if self.opt.lambda_CE > 0.0:
            _, real_B_encoded_label = torch.max(self.real_B_encoded, dim=1)
            self.loss_G_CE = self.criterionCE(self.fake_B_encoded, real_B_encoded_label) * self.opt.lambda_CE
        else:
            self.loss_G_CE = 0.0

        self.loss_G = self.loss_G_GAN + self.loss_G_GAN2 + self.loss_G_L1 + self.loss_kl + self.loss_kl_rev + self.loss_G_CE
